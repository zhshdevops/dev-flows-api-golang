 ...
 * 2016-10-26  @author mengyuan
 *
 * SAMPLE
 trans := transaction.New()
 trans.Do(
 	if name, err := app.LockRow(trans.O(), params[":cluster"], c.Namespace, body.Name); err != nil {
 		trans.Rollback("lock app", body.Name, "failed.", err)
 	} else if name != "" {
 		trans.Rollback("app", body.Name, "exist")
 	}
 }).Do(func() {
 	if err := app.Insert(trans.O()); err != nil {
 		trans.Rollback("insert table "+app.TableName()+" failed.", err)
 	}
 }).Done()
 )
*/
