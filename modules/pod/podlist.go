...
func GetPodList(client v1core.CoreClient, nsQuery *common.NamespaceQuery, dsQuery *dataselect.DataSelectQuery) (*PodList, error) {
...
func GetPodListBySelector(client v1core.CoreClient, nsQuery *common.NamespaceQuery, dsQuery *dataselect.DataSelectQuery, labelSet, fieldSet map[string]string) (*PodList, error) {
	glog.V(4).Infof("Getting pod list by LabelSelector and fieldSet in the cluster\n")
	option := &v1.ListOptions{
		LabelSelector: labels.SelectorFromSet(labels.Set(labelSet)),
		FieldSelector: fields.SelectorFromSet(fields.Set(fieldSet)),
	}
...
func GetPodListByOption(client v1core.CoreClient, nsQuery *common.NamespaceQuery, dsQuery *dataselect.DataSelectQuery, option *v1.ListOptions) (*PodList, error) {
	channels := &common.ResourceChannels{
...
func GetCountByNamespace(k8sList []*k8sclient.ClientSet, namespaceList []string) (int, error) {
	...
	for _, namespace := range namespaceList {
		for _, k8s := range k8sList {
			podList, err := k8s.CoreClient.Pods(namespace).List(v1.ListOptions{
