...
func GetPodListByOption(client v1core.CoreV1Client, nsQuery *common.NamespaceQuery, dsQuery *dataselect.DataSelectQuery, option *metav1.ListOptions) (*PodList, error) {
	channels := &common.ResourceChannels{
...
func GetCountByNamespace(k8sList []*k8sclient.ClientSet, namespaceList []string) (int, error) {
	...
	for _, namespace := range namespaceList {
		for _, k8s := range k8sList {
			podList, err := k8s.CoreV1Client.Pods(namespace).List(metav1.ListOptions{
