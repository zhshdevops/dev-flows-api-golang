...
func GetPodInfo(current int32, desired int32, pods []v1api.Pod) PodInfo {
	result := PodInfo{
		Current:  current,
		Desired:  desired,
	...
	for _, pod := range pods {
		...
		case v1api.PodRunning:
			result.Running++
		case v1api.PodPending:
			result.Pending++
		case v1api.PodFailed:
			result.Failed++
		case v1api.PodSucceeded:
			result.Succeeded++
		}
	}
