...
func (u *UserModel) FindByName(orms ...orm.Ormer) (uint32, error) {
	var o orm.Ormer
	if len(orms) != 1 {
		o = orm.NewOrm()
	} else {
	...
	err := o.QueryTable(u.TableName()).Filter("user_name", u.Username).One(u)
	return sqlstatus.ParseErrorCode(err)
}

func (u *UserModel) FindByToken(orms ...orm.Ormer) (uint32, error) {
	var o orm.Ormer
	if len(orms) != 1 {
		o = orm.NewOrm()
	} else {
	...
	err := o.QueryTable(u.TableName()).Filter("user_name", u.Username).Filter("api_token", u.APIToken).One(u)
	return sqlstatus.ParseErrorCode(err)
}
...
func (u *UserModel) Update() (uint32, error) {
	o := orm.NewOrm()
	_, err := o.Update(u)
	return sqlstatus.ParseErrorCode(err)
}
